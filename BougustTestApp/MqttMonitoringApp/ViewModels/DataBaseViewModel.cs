...
namespace MqttMonitoringApp.ViewModels
{
    public class DataBaseViewModel : Conductor<object>
    {
        ...
        private void InsertDataBase(string message)
        {
            ...
            {
                string strInsQurey = " INSERT INTO smarthometbl " +
                                     " (Dev_Id, " + 
                                     " Curr_Time, " + 
                                     " Temp, " +
                                     " Humid, " +
                ...
                {
                    conn.Open();
                    MySqlCommand cmd = new MySqlCommand(strInsQurey, conn);
                    ...
                    paramCurrTime.Value = currDatas["Curr_Time"];
                    ...
                    if (cmd.ExecuteNonQuery() == 1) UpdateText("[DB] Inserted");
                    else UpdateText("[DB] Failed");
                }
                catch (Exception ex)
                {
